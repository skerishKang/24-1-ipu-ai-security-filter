...
async function main() {
  ...
  try {
    await runTest(browser, "첫 로드 시 backend 연결 상태를 표시한다", async (page) => {
      await page.goto(FRONTEND_URL);
      await page.waitForLoadState("networkidle");
      await expectTextIncludes(page.getByTestId("session-source"), "backend");
    });

    await runTest(browser, "텍스트 요청 후 session/source 에 backend 가 표시된다", async (page) => {
      await page.goto(FRONTEND_URL);
      await page.waitForLoadState("networkidle");
    ...
    await runTest(browser, "txt 파일 업로드 후 session/source 에 backend-file 이 표시된다", async (page) => {
      await page.goto(FRONTEND_URL);
      await page.waitForLoadState("networkidle");
